...
for t in umich_tweets:
	...
	x = cursor.fetchall()
	tup1 = t["user"]["id_str"], t["user"]["screen_name"], t["user"]["favourites_count"], t["user"]["description"]
	if len(x) == 0:
		try:
...
retweets = cursor.execute('SELECT * from Tweets WHERE retweets >10')
